...
class SearchPipeline:
    ...
    def run(self) -> None:
        jobs = self._crawl()
        processed_jobs = self._process_jobs_with_ai(jobs)
        self._handle_search_task(processed_jobs)

    def _get_user_cv_context(self) -> str:
        """Get the user CV context."""
        result = ""
        result += f"Name: {self.user_cv.get('name')}\n"
        result += f"Title: {self.user_cv.get('title')}\n"
        result += f"About: {self.user_cv.get('about')}\n"
        result += f"Highlights: {self.user_cv.get('highlights')}\n"
        result += "Experience:\n"
        for experience in self.user_cv.get('experience', []):
        ...
        result += f"Skills: {','.join(self.user_cv.get('skills', []))}\n"
        result += "Education:\n"
        for education in self.user_cv.get('education', []):
    ...
    def _get_job_context(self, job_data: dict) -> str:
        """Get the job context."""
        result = ""
        result += f"Title: {job_data.get('title')}\n"
    ...
    def _get_system_prompt(self, job_data: dict) -> str:
        ...
        JOB DESCRIPTION:
        {self._get_job_context(job_data)}
        """
    ...
    def _crawl(self):
        ...
        process.crawl(LinkedInSpider, **self.spider_config)
        process.start()

        return jobs


    def _process_jobs_with_ai(self, jobs: List[dict]):
        """
        Process jobs with AI agent.
        """
        processed_jobs: List[dict] = []
        conf = AIProviderConfig(**self.ai_config)
